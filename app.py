    ...
    if scan and url:
        with st.spinner("Analyzing..."):
            ...
            if md:
                ...
                pred = md['ensemble'].predict(arr)[0]
                prob = md['ensemble'].predict_proba(arr)[0]
                conf = prob[pred] * 100
            else:
...
# ── Team ──────────────────────────────────────────────────────
st.markdown("---")
st.markdown('<div class="section-title">👥 Project Team — UPES Dehradun</div>', unsafe_allow_html=True)
team = [("Ananya Chaturvedi","500121772"),("Aditya Rathee","500121162"),
        ("Arnav Sagar","500119739"),("Eshaan Parmaar","500119487")]
for col, (name, sid) in zip(st.columns(4), team):
    col.markdown(f"""
    <div class="team-card">
      <div style="font-size:1.8rem">👤</div>
      <div class="team-name">{name}</div>
      <div class="team-id">{sid}</div>
      <div class="team-role">CS · Cyber Security</div>
    </div>""", unsafe_allow_html=True)

st.markdown('<br><div style="text-align:center;color:#334155;font-size:.78rem">PhishGuard Pro · Minor Project · UPES · Guided by Mr. Rochak Bajpai</div><br>', unsafe_allow_html=True)